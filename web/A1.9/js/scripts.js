...
function tocarAudio2(event, audioId) {
...
function carregaaula(id, pausavideo) {
    if (id == 'a_menu') {
...
//Monta Próximo e Anterior
    ...
    $('#anterior').off('click');
    if (anteriorLink == '') {
        $('#proximo').attr('onclick', 'carregaaula("a_' + proximoLink + '",true)');
        $('#proximo').show();

    } else if (proximoLink == '') {
        $('#anterior').attr('onclick', 'carregaaula("a_' + anteriorLink + '",true)');
        $('#anterior').show();
...
$('.multipla-escolha>form>div>label').click(function () {
...
//        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
...
//Verifica Texto Simples
$('input[type="text"]:not(".conjunto")').change(function () {
    var valorcorreto = $('#' + this.name + "-sol").val();
	var valorcorreto2=$('#' + this.name + "-sol").attr('value2');
	if (typeof valorcorreto2 == typeof undefined ) {
  valorcorreto2="";
}
    if (this.value.toLowerCase() == valorcorreto.toLowerCase()||this.value.toLowerCase() == valorcorreto2.toLowerCase()) {
        $('.feedback_questao').hide();
		$('.feedback_questao').css('background-color', '#72b5a4e3');
        $('.feedback_questao').html('Parabéns! Você acertou.');
        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
        ...
        $('.feedback_questao').hide();
        $('.feedback_questao').css('background-color', '#ffabb4e3');
        $('.feedback_questao').html('Tente novamente.<br>'+valorcorreto);
        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
        return false;
    }
//
...
//Verifica Preenche Coluna
function verificaPfrase(id1,id2) {
	if(id2==0){
		 var valorcorreto1 = $('#' + id1 + "-sol").val();
	
    if (($('#'+id1).val().toLowerCase() == valorcorreto1.toLowerCase())) {
        $('.feedback_questao').hide();
		$('.feedback_questao').css('background-color', '#72b5a4e3');
        $('.feedback_questao').html('Parabéns! Você acertou.');
        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
		$('#' + id1).css('border','1px solid green');
        return false;

    } else {
        this.value = '';
        $('.feedback_questao').hide();
		$('#' + id1).css('border','1px solid red');
        $('.feedback_questao').css('background-color', '#ffabb4e3');
        $('.feedback_questao').html('Tente novamente.');
        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
        return false;
    ...
    var valorcorreto1 = $('#' + id1 + "-sol").val();
	var valorcorreto2 = $('#' + id2 + "-sol").val();
    if (($('#'+id1).val().toLowerCase() == valorcorreto1.toLowerCase())&&($('#'+id2).val().toLowerCase() == valorcorreto2.toLowerCase())) {
        $('.feedback_questao').hide();
		$('.feedback_questao').css('background-color', '#72b5a4e3');
        $('.feedback_questao').html('Parabéns! Você acertou.');
		$('#' + id1).css('border','1px solid green');
		$('#' + id2).css('border','1px solid green');
        $('.feedback_questao').show();setTimeout(function() {$(".feedback_questao").hide('blind', {}, 500)}, 5000);
        return false;
		...
		if(($('#'+id2).val().toLowerCase() != valorcorreto2.toLowerCase())){
...
function verificaPfraseN(classe) {
	errado=0;
	$('.'+classe).each(function(){
		if($(this).val().toLowerCase()==$('#'+$(this).attr('id')+'-sol').val().toLowerCase()){
			$(this).css('border','1px solid green');	
		}
		else{
		$(this).css('border','1px solid red');		
		$(this).val($(this).val()+ "("+$('#'+$(this).attr('id')+'-sol').val()+")")
		errado++;
		}
...
$(document).ready(function () {
	...
	//Randomiza opcoes drag
	$(".opcoes").randomize();
	$(".multipla-escolha form").randomize();
	$( function() {
...
$( this ).attr('identificador',ui.draggable.find('img').attr('identificado'));
}
});


//Dragdrop legenda
$( ".dragdroplegenda .ui-droppable" ).droppable( {
drop: function( event, ui ) {
	//ui.draggable.hide();
$( this ).addClass( "ui-state-highlight" );
$( this ).attr('value',ui.draggable.attr('sol'));
$( this ).attr('identificador',ui.draggable.find('img').attr('identificado'));
}
});
//Dragdrop 3 colunas
$( ".m3colunas .ui-droppable" ).droppable( {
drop: function( event, ui ) {
	//ui.draggable.hide();
ui.draggable.addClass( "ui-state-highlight" );
ui.draggable.attr('sol2',$(this).attr('sol'));
...
function verificaligacoluna(botao) {
    ...
    for (var i = 0; i < itemOrder.length; i++) {
        ...
        if ($('#' + itemOrder[i]).text().replace(/(?:\r\n|\r|\n)/g, '') === $('#' + vinculo + (i) + 'sol').attr('sol')) {
...
if($(this).text()==$(this).attr('sol')||$(this).attr('value')==$(this).attr('sol')){
$(this).addClass('correct');
//Remover após classe correct funcionar
}else{
	$(this).addClass('wrong');
	errado++;
 //Remover após classe correct funcionar
...
function verificaDragDropImagem(){
	errado=0;
	$('.ui-state-highlight').each(function(){
if($(this).attr('identificador')==$(this).attr('sol')){
$(this).addClass('correct');
//Remover após classe correct funcionar
}else{
	$(this).addClass('wrong');
...
function reiniciaDragDrop(){
	$('.ui-draggable-handle').removeAttr('style');
	$('.ui-draggable-handle').attr('style','position:relative');
$('.ui-droppable').removeAttr('value');
$('.ui-droppable').removeAttr('identificado');

$('.ui-droppable').removeClass('ui-state-highlight');
$('.ui-droppable').removeClass('wrong');
$('.ui-droppable').removeClass('correct');
$('.feedback_questao').hide();

}

function verificaDitado(classe) {
	errado=0;
	$('.'+classe).each(function(){
		if($(this).val().toLowerCase()==$('#'+$(this).attr('id')+'-sol').val().toLowerCase()){
			$(this).css('border','1px solid green');	
	...
	function verificaDragDropColuna(){
errado=0;
$('.ui-state-highlight').each(function(){
if($(this).attr('sol2')==$(this).attr('sol')||$(this).attr('value')==$(this).attr('sol')){
$(this).addClass('correct');
//Remover após classe correct funcionar
}else{
	$(this).addClass('wrong');
 ...
 //Remover após classe correct funcionar
	...
	function verificaDragDropLegenda(){
			errado=0;
	$('.ui-state-highlight').each(function(){
if($(this).attr('value')==$(this).attr('sol')){
$(this).addClass('correct');
//Remover após classe correct funcionar
}else{
	$(this).addClass('wrong');
